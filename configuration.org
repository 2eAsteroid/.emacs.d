...
#+begin_src emacs-lisp :tangle "modules/essentials.el"
  (setq-default ad-redefinition-action 'warn            ; When we redefine an active function then deactivate it, it will accept the redefinition as the new definition, but warn us. Might change to 'accept
                ...
                delete-old-versions t                   ; Don't ask for confirmation for removing excess backup versions
                ...
                backup-by-copying t                     ; Instead of symlinking, make a copy for the backup.
...
#+end_src
** The =writing.el= Module
Although I use Emacs for coding, I also use it as a dedicated writing environment. These are some things that massively improve the experience.
*** =writing.el='s Olivetti Configuration
Olivetti is an Emacs minor mode that balances window margins to keep text centered.
#+begin_src emacs-lisp :tangle "modules/writing.el"
  (straight-use-package
   '(olivetti :type git
              :host github
              :repo "rnkn/olivetti"
  ...
  (add-function-to-hooks #'olivetti-mode '(text-mode-hook
                                           ...
                                           org-mode-hook))
#+end_src
*** =writing.el='s Hydra Configuration
#+begin_src emacs-lisp :tangle "modules/writing.el"
  (straight-use-package
   '(hydra :type git
           :host github
  ...
  (require 'hydra)
#+end_src
*** =writing.el='s Provide
#+begin_src emacs-lisp :tangle "modules/writing.el"
  (provide 'writing)
#+end_src
** The =magic.el= Module
This is for magit.
*** =magic.el='s Magit Core Config
#+begin_src emacs-lisp :tangle "modules/magic.el"
  (straight-use-package
   '(magit :type git
           :host github
           :repo "magit/magit"
...
*** =soul-theme-theme.el='s Theme Documentation
This is needed for packages, I think. It's basically just metadata for the theme.
#+begin_src emacs-lisp :tangle "config/soul-theme-theme.el"
  ...
  ;;; Code:
#+end_src
*** =soul-theme-theme.el='s Requires
This theme requires two packages, which are needed for the loading of this theme.
#+begin_src emacs-lisp :tangle "config/soul-theme-theme.el"
  (eval-when-compile
    (require 'cl-lib))
...
#+end_src
*** =soul-theme-theme.el='s Autothemer Theme Defining
I use autothemer for my theme, which is basically a wrapper (?) for defface and such.
#+begin_src emacs-lisp :tangle "config/soul-theme-theme.el"
  (autothemer-deftheme
   soul-theme "Light theme that seeks to emulate paper sort of."

#+end_src
*** =soul-theme-theme.el='s Color Palette Definitions
In autothemer, you can define colors to use in your later face definitions, which makes it easier to color themes right. I still have to fix the 24bit -> 8bit thing though, to make it work for terminals.
#+begin_src emacs-lisp :tangle "config/soul-theme-theme.el"
...
#+end_src
*** =soul-theme-theme.el='s Default Faces
These are faces that come with vanilla Emacs.
#+begin_src emacs-lisp :tangle "config/soul-theme-theme.el"
  ;; Customize faces
  (
...
#+end_src
*** =soul-theme-theme.el='s Font Lock and Friends
Although I primarily use treesitter for highlighting, sometimes font lock is faster / more available.
#+begin_src emacs-lisp :tangle "config/soul-theme-theme.el"
  ;; ;; Font lock
...
#+end_src
*** =soul-theme-theme.el='s Elfeed Faces
This is kind of incomplete, but I don't use Elfeed that much for now anyways.
#+begin_src emacs-lisp :tangle "config/soul-theme-theme.el"
  ;;elfeed
  (elfeed-search-feed-face              (:foreground spirit))
  (elfeed-search-tag-face               (:foreground ectoplasm))
#+end_src
*** =soul-theme-theme.el='s Message (Mail) Faces
#+begin_src emacs-lisp :tangle "config/soul-theme-theme.el"
  ;; message colors
...
#+end_src
*** =soul-theme-theme.el='s Org Faces
#+begin_src emacs-lisp :tangle "config/soul-theme-theme.el"
  ;; org-mode
